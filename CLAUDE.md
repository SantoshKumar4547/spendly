...
- Templates: Jinja2 with `url_for()` for every internal link — never hardcode
  URLs
- Route functions: one responsibility only — fetch data, render template, done
- DB queries: always use parameterized queries (`?` placeholders) — never
  f-strings in SQL
- Error handling: use `abort()` for HTTP errors, not bare
  `return "error string"`
...
**Do not implement a stub route unless the active task explicitly targets that
step.**
...
- **`database/db.py` is currently empty** — do not assume helpers exist until
